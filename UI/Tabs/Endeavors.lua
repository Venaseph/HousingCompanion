...
local function GetNextXPForTask(task)
    ...
    if not HC.EndeavorTracker then return 0 end
...
function HC.UI.Tabs:CreateEndeavors(parent)
    ...
    local tasksHeader = HC.UI:CreateSectionHeader(container, "Endeavor Tasks")
    ...
    local ApplyContainerColors = HC.UI:AddAtlasBackground(taskListContainer)
    ApplyContainerColors()

    local _, scrollContent = HC.UI:CreateScrollFrame(taskListContainer)
    ...
    function container:UpdateTaskList(tasks, forceUpdate)
        ...
        local rankings = {}
        if showRewardsHighlight and HC.EndeavorTracker then
            rankings = HC.EndeavorTracker:GetTaskRankings() or {}
        end
        ...
        for i, task in ipairs(displayTasks) do
            ...
            if not row then
                row = HC.UI:CreateTaskRow(self.scrollContent)
                self.taskRows[i] = row
            end
    ...
    function container:ShowEmptyState()
        ...
        local isViewingActive = HC.EndeavorTracker and HC.EndeavorTracker:IsViewingActiveNeighborhood()
