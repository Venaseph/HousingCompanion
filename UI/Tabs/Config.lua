...
local function GetColors()
    return HC.Constants:GetThemeColors()
end

function HC.UI.Tabs:CreateConfig(parent)
    ...
    local header = HC.UI:CreateSectionHeader(container, "Settings")
    ...
    local ApplyPanelColors = HC.UI:AddAtlasBackground(scrollContainer)
    ApplyPanelColors()

    local _, scrollContent = HC.UI:CreateScrollFrame(scrollContainer)
    ...
    HC.Theme.ApplyFont(discordHint, discordColors, "small")
    ...
    local function CreateCheckbox(labelText, configKey, description)
        ...
        if description then
            ...
            HC.Theme.ApplyFont(desc, C, "small")
        ...
        checkbox:SetScript("OnClick", function(self)
            ...
            if configKey == "showMinimapButton" then
                if HC.Minimap then
                    HC.Minimap:UpdateVisibility()
                end
            end

            -- Special handling for dashboard button
            if configKey == "showDashboardButton" then
                if HC.UpdateDashboardButtonVisibility then
                    HC:UpdateDashboardButtonVisibility()
                end
            end
        end)
        ...
        function row:UpdateColors()
            ...
            HC.Theme.ApplyFont(self.label, colors)
            if self.desc then
                self.desc:SetTextColor(colors.text_dim.r, colors.text_dim.g, colors.text_dim.b)
                HC.Theme.ApplyFont(self.desc, colors, "small")
            end
        end
    ...
    local quotesHeader = HC.UI:CreateSectionHeader(settingsPanel, "Squirrel Quotes")
    ...
    local altSharingHeader = HC.UI:CreateSectionHeader(settingsPanel, "Alt Sharing")
    ...
    HC.Theme.ApplyFont(altShareLabel, altShareColors)
    ...
    HC.Theme.ApplyFont(altShareDesc, altShareColors, "small")
    ...
    HC.Theme.ApplyFont(groupByAccountLabel, groupByAccountColors)
    ...
    -- Get/Set from state
    local groupByAccountState = HC.Store:GetState()
    ...
    HC.Theme.ApplyFont(mainCharLabel, mainCharColors)
    ...
    local mainCharDropdown = HC.UI:CreateDropdown(mainCharRow, {
        width = 140,
        height = 22,
        onSelect = function(key, data)
            local mainChar = key ~= "" and key or nil
            HC.Store:Dispatch("SET_MAIN_CHARACTER", { mainCharacter = mainChar })
            -- Trigger re-broadcast
            if HC.AltSharing and HC.AltSharing.BroadcastIfEnabled then
    ...
    HC.Theme.ApplyFont(privacyNote, mainCharColors, "small")
    ...
    local appearanceHeader = HC.UI:CreateSectionHeader(settingsPanel, "Appearance")
    ...
    HC.Theme.ApplyFont(themeLabel, themeColors)
    themeLabel:SetText("Theme")
    themeLabel:SetTextColor(themeColors.text.r, themeColors.text.g, themeColors.text.b)
    themeRow.label = themeLabel

    local themeDropdown = HC.UI:CreateDropdown(themeRow, {
        width = 160,
        height = 22,
        onSelect = function(key, data)
            -- Update theme
            HC.Store:Dispatch("SET_CONFIG", { key = "theme", value = key })
    ...
    local themeItems = {}
    for _, themeKey in ipairs(HC.Constants.ThemeOrder) do
        table.insert(themeItems, {
            key = themeKey,
            label = HC.Constants.ThemeDisplayNames[themeKey] or themeKey
        })
    end
    ...
    -- Build font options
    local fontItems = {}
    for _, fontKey in ipairs(HC.Constants.FontOrder) do
        table.insert(fontItems, {
            key = fontKey,
            label = HC.Constants.FontDisplayNames[fontKey] or fontKey
        })
    end
    fontDropdown:SetItems(fontItems)

    -- Set current font as selected
    local currentFont = HC.Store.state.config.fontFamily or "ARIALN"
    local currentFontName = HC.Constants.FontDisplayNames[currentFont] or currentFont
    ...
    HC.Theme.ApplyFont(uiScaleLabel, uiScaleColors)
    ...
    local uiScaleUpBtn = HC.UI:CreateButton(uiScaleRow, "+", 24, 22)
    ...
    HC.Theme.ApplyFont(uiScaleValue, uiScaleColors)
    local currentUIScale = HC.Store.state.config.uiScale or 1.0
    ...
    local uiScaleDownBtn = HC.UI:CreateButton(uiScaleRow, "-", 24, 22)
    ...
    HC.Theme.ApplyFont(opacityLabel, opacityColors)
    ...
    -- Increase button (rightmost)
    local opacityUpBtn = HC.UI:CreateButton(opacityRow, "+", 24, 22)
    ...
    HC.Theme.ApplyFont(opacityValue, opacityColors)
    local currentOpacity = HC.Store.state.config.bgOpacity or 0.9
    ...
    -- Decrease button (left of display)
    local opacityDownBtn = HC.UI:CreateButton(opacityRow, "-", 24, 22)
    ...
    HC.Theme.ApplyFont(versionInfo, C, "small")
    ...
    HC.Theme.ApplyFont(fontCredit, C, "small")
    ...
    HC.EventBus:Register("HC_THEME_UPDATE", function()
        ...
        if container.discordHint then
            ...
            HC.Theme.ApplyFont(container.discordHint, colors, "small")
        end
    end)
