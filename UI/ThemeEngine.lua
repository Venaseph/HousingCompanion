...
HC.Theme.registry = setmetatable({}, { __mode = "k" })
HC.Theme.currentScheme = nil  -- Set during initialization

-- ============================================================================
-- CENTRALIZED BACKDROP
-- ============================================================================

HC.Theme.BACKDROP_FLAT = {
...
function HC.Theme:Initialize()
    ...
    self.currentScheme = HC.Colors.Schemes[themeName] or HC.Colors.Schemes.HousingTheme
...
function HC.Theme:Register(widget, widgetType)
...
function HC.Theme:GetScheme()
    return self.currentScheme or HC.Colors.Schemes.SolarizedDark
end
...
local function ApplyFont(fontString, scheme, fontType)
    ...
    if f then
        local fontFile = HC.Constants:GetFontFile()
        local fontScale = 0
        if HC.Store and HC.Store.state and HC.Store.state.config then
            fontScale = HC.Store.state.config.fontScale or 0
        end
...
HC.Theme.ApplyTextShadow = ApplyTextShadow
HC.Theme.ApplyFont = ApplyFont

-- Get background opacity multiplier from config
local function GetBgOpacity()
    if HC.Store and HC.Store.state and HC.Store.state.config then
        return HC.Store.state.config.bgOpacity or 0.9
    end
    return 0.9
end
HC.Theme.GetBgOpacity = GetBgOpacity
...
HC.Theme.ApplyAtlasBackground = ApplyAtlasBackground
HC.Theme.ApplyAtlasHeader = ApplyAtlasHeader
HC.Theme.HideAtlasTextures = HideAtlasTextures
        ...
        if f.pointsBg then
            if c.atlas and c.atlas.xpBanner then
                ...
                if not f.pointsBg:GetBackdrop() then
                    f.pointsBg:SetBackdrop(HC.Theme.BACKDROP_FLAT)
                end
    ...
    TabButton = function(btn, c)
        ...
        if c.atlas and c.atlas.tabActive and c.atlas.tabInactive then
            ...
            if not btn:GetBackdrop() then
                btn:SetBackdrop(HC.Theme.BACKDROP_FLAT)
            end
