...
local Tracker = HC.HousingTracker
...
function Tracker:OnEvent(event, ...)
    ...
    elseif event == "CURRENCY_DISPLAY_UPDATE" then
        ...
        local COMMUNITY_COUPONS = HC.Constants and HC.Constants.CURRENCY_IDS and HC.Constants.CURRENCY_IDS.COMMUNITY_COUPONS or 3363
...
function Tracker:OnHouseListUpdated(houseInfoList)
    ...
    if selectedHouse and selectedHouse.houseGUID then
        HC.Store:Dispatch("SET_HOUSE_GUID", { houseGUID = selectedHouse.houseGUID })
...
function Tracker:OnHouseLevelFavorUpdated(houseLevelFavor)
    ...
    if not houseLevelFavor or type(houseLevelFavor) ~= "table" then
        HC.Store:Dispatch("SET_HOUSE_LEVEL", {
            level = 0,
            xp = 0,
            xpForNextLevel = 0,
        })
